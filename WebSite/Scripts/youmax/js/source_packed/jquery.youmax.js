...
//Youmax 5.0 - http://www.codehandling.com/2015/02/youmax-50-complete-solution-for.html
...
/*>>iframe*/
 ...
 //Youmax 5.0 - http://www.codehandling.com/2015/01/youmax-40-bring-youtube-to-your-users.html
...
(function ($) {
	...
	getVideoComments = function (videoId,$youmaxContainer) {
		//console.log('inside getPlaylistVideos');
		//var pageTokenUrl = "";
		var loadMoreFlag = false;
		var youmax_global_options = $youmaxContainer.data('youmax_global_options');
		...
		apiVideoCommentsURL = "https://gdata.youtube.com/feeds/api/videos/"+videoId+"/comments?v=2&alt=json&start-index="+startIndex+"&max-results="+maxResults+"&key="+apiKey;
		
		//console.log('getPlaylistVideos apiVideoCommentsURL-'+apiVideoCommentsURL);
	...
	initYoumax = function($youmaxContainer) {
		...
		//load more
		$youmaxContainer.append('<button id="youmax-load-more-div">LOAD MORE</button>');
	...
	*/
	...
	displayChannelHeader = function(response,$youmaxContainer) {
		...
		$youmaxContainer.find('#youmax-tabs').prepend('<span id="playlists_'+channelId+'" class="youmax-tab" >Playlists</span>');
		$youmaxContainer.find('#youmax-select').prepend('<option value="playlists_'+channelId+'" class="youmax-option-highlight" >Playlists</span>');
	...
	insertVideoComments = function(response,loadMoreFlag,$youmaxContainer) {
		...
		//console.log('youmax_global_options.imagesFolderPath-'+youmax_global_options.imagesFolderPath)
		...
		var commentArray = response.feed.entry;
		var userIdArray = [];
		...
		var startIndex = parseInt($loadCommentsButton.data('start-index'));
		startIndex += commentArray.length;
		$loadCommentsButton.data('start-index',startIndex);
		
		for(var i=0; i<commentArray.length; i++) {
			comment = commentArray[i].content.$t;
			if(null==comment||comment=="") {
				continue;
			}
			commentPublished = commentArray[i].published.$t;
			authorName = commentArray[i].author[0].name.$t;
			...
			//id="'+authorNameAsId+'"
			$youmaxCommentHolder.append('<div  class="youmax-video-comment '+authorNameAsClass+'"><div class="youmax-from"><div class="youmax-from-img" style="background-image:url(\''+youmax_global_options.imagesFolderPath+'/unknown.jpg\');"></div><div class="youmax-from-name">'+authorName+'</div><div class="youmax-published">'+getDateDiff(commentPublished)+' ago</div></div><div class="youmax-comment"><span class="youmax-comment-content">'+comment+'</span><div></div>');
	...
	insertChannelPlaylists = function(response,loadMoreFlag,$youmaxContainer) {
		...
		for(var i=0; i<playlistArray.length; i++) {
			...
			$youmaxContainerList.append('<li id="'+playlistId+'" ><img src="'+playlistThumbnail+'"><div class="youmax-playlist-video-count-wrapper"><div class="youmax-playlist-video-count-box"><span class="youmax-playlist-video-count">'+videoCount+'</span><br>VIDEOS<br><div class="youmax-playlist-line-wrapper"><span class="youmax-playlist-line"></span><br><span class="youmax-playlist-line"></span><br><span class="youmax-playlist-line"></span></div></div></div><p><span class="youmax-video-list-title">'+playlistTitle+'</span><span class="youmax-video-list-views">'+getDateDiff(playlistUploaded)+' ago</span></p></li>');
	...
	registerPopup = function($youmaxContainer) {
		...
		if(youmax_global_options.displayVideo=="popup") {
			...
			frame_source = "http://www.youtube.com/embed/%id%?rel=0";
	...
	showLoader = function($youmaxContainer) {
		...
		$youmaxContainer.find('#youmax-video-list-div>ul').append('<div style="text-align:center; height:200px; font:14px Calibri;"><br><br><br><br><br><br>loading...<br><br><br><br><br><br></div>');
		$youmaxContainer.find('#youmax-showing-title').empty().hide();
	},
	...
	loadMoreComments = function($youmaxContainer) {
		...
		$youmaxMoreButton = $youmaxContainer.find(".youmax-encloser-comment-button.youmax-more-button");
		$youmaxMoreButton.addClass('youmax-load-more-comments-clicked');
		
		
		//$youmaxCommentHolder = $youmaxContainer.find("#youmax-encloser-comment-holder");
		var startIndex = parseInt($youmaxMoreButton.data('start-index'));
		//console.log('loadMoreComments startIndex-'+$youmaxMoreButton.data('start-index'));
		//console.log('loadMoreComments startIndex-'+startIndex);
